...
/**
 * Diceware Passphrase Generator: pure-logic library.
 *
 * Generates a memorable passphrase by drawing words from the EFF Short
 * Wordlist #1 (1,296 words, average length ~4.5 characters). Each word
 * contributes log2(1296) bits of entropy, so a six-word passphrase carries
 * about 62 bits of resistance against brute force. Memorable beats random
 * for human-typed passwords because users do not write down what they can
 * recall, and a passphrase they can recall is one they will not weaken by
 * sticking on a Post-it.
 *
 * Randomness comes from crypto.getRandomValues with rejection sampling, so
 * there is no modulo bias. The library never calls Math.random.
 *
 * Wordlist source: https://www.eff.org/dice (EFF Short Wordlist #1, 2016).
 */

// EFF Short Wordlist #1 (1,296 words). Baked in at build time. Do not edit
// by hand: regenerate from the upstream list if it ever changes.
var WORDLIST = [
...
var ENTROPY_PER_WORD = Math.log2(WORDLIST.length); // log2(1296) ≈ 10.3398

// --- Random sampling -----------------------------------------------------

function defaultRand32() {
  // 32-bit unsigned random integer from the Web Crypto CSPRNG. Falls back
  // to throwing if crypto.getRandomValues is unavailable. Math.random is
  // never used.
...
function unbiasedRandomIndex(range, src) {
  // Rejection sampling against the largest multiple of `range` below 2^32.
  // Anything above the cutoff is in the biased tail, so we redraw. This
  // gives a perfectly uniform distribution across [0, range).
  var rand32 = src || defaultRand32;
  var TWO_32 = 0x100000000; // 2^32
  var limit = Math.floor(TWO_32 / range) * range;
  var x;
  do {
    x = rand32() >>> 0;
  } while (x >= limit);
...
function pickWords(count, src) {
  var out = [];
  for (var i = 0; i < count; i++) {
    out.push(WORDLIST[unbiasedRandomIndex(WORDLIST.length, src)]);
  }
  return out;
}
...
function applyCapitalisation(words, mode, randomPicker) {
  if (mode === 'first') {
    return words.map(capFirst);
  }
  if (mode === 'random') {
    // Pick exactly one word at random and capitalise it. The picker
    // returns an integer in [0, words.length). Default uses unbiased
    // sampling against the wordlist length.
    var pick = randomPicker || function () { return unbiasedRandomIndex(words.length); };
    var idx = pick(words.length);
    return words.map(function (w, i) { return i === idx ? capFirst(w) : w; });
  }
  // 'none' or anything unrecognised: leave alone.
  return words.slice();
}
...
function renderPassphrase(words, separator) {
  var sep = Object.prototype.hasOwnProperty.call(SEPARATORS, separator)
...
function entropyBits(wordCount) {
  return wordCount * ENTROPY_PER_WORD;
}

function resistanceSummary(bits) {
  // Honest, banded reading. Rough thresholds align with public guidance:
  // < 40 bits is trivially crackable; ~50 bits resists casual attackers;
  // ~60 bits resists organised crime budgets for years; ~70+ bits is
  // resistant to a state-funded attacker for a very long time. These
  // numbers assume the attacker knows the wordlist and the structure, so
  // they are the correct "Diceware" entropy figures, not naive
  // character-class approximations.
  if (bits < 30) {
    return 'Weak. A laptop can guess this in seconds. Use more words.';
  }
  if (bits < 45) {
    return 'Fragile. A modern GPU can grind through this in hours to days. Add words.';
  }
  if (bits < 55) {
    return 'Moderate. Stands up to opportunistic attackers but not a determined one. Fine for low-stakes accounts.';
  }
  if (bits < 65) {
    return 'Strong. Resists organised crime budgets for many years. A reasonable floor for important accounts.';
  }
  if (bits < 80) {
    return 'Very strong. Resists a state-funded attacker for many years on current hardware.';
  }
  return 'Overkill. Comfortably beyond any realistic offline attack on present or near-future hardware.';
}
...
function generate(options) {
  var opts = options || {};
  var count = opts.wordCount || 6;
  var separator = opts.separator || 'hyphen';
  var capitalisation = opts.capitalisation || 'none';
  var src = opts.randSource;

  var words = pickWords(count, src);
  var capped = applyCapitalisation(words, capitalisation);
  var phrase = renderPassphrase(capped, separator);
  var bits = entropyBits(count);
  return {
    words: capped,
    passphrase: phrase,
    entropyBits: bits,
    entropyPerWord: ENTROPY_PER_WORD,
    wordlistSize: WORDLIST.length,
    resistance: resistanceSummary(bits)
  };
}
...
var exported = {
  ...
  unbiasedRandomIndex: unbiasedRandomIndex,
  pickWords: pickWords,
  applyCapitalisation: applyCapitalisation,
  renderPassphrase: renderPassphrase,
