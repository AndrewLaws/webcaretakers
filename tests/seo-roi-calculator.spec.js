...
test.describe('SEO ROI Calculator page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/calculators/finance/seo-roi-calculator/');
  });

  test('has the expected h1', async ({ page }) => {
    await expect(page.getByRole('heading', { level: 1, name: 'SEO ROI Calculator' })).toBeVisible();
  });
  ...
  test('results panel shows all key lines', async ({ page }) => {
    await page.locator('[data-calculate]').click();
    await expect(page.locator('[data-line-additional-sessions]')).not.toHaveText('—');
    await expect(page.locator('[data-line-additional-revenue]')).not.toHaveText('—');
  ...
  test('currency toggle switches symbols on labels and results', async ({ page }) => {
    ...
    await page.locator('[data-calculate]').click();
    const revenue = await page.locator('[data-line-additional-revenue]').textContent();
    expect(revenue).toContain('$');
  });

  test('profitable scenario shows positive status', async ({ page }) => {
    // Default inputs: £1500 cost, 5k→10k sessions, 2% conversion, £75/conversion
    // Additional revenue = 5000 × 0.02 × 75 = £7500 > £1500 cost → profitable
    await page.locator('[data-calculate]').click();
  ...
  test('loss scenario shows not-yet-profitable status', async ({ page }) => {
    // Set tiny projected growth that won't cover cost
    await page.fill('[data-projected-sessions]', '5100');
    await page.fill('[data-conversion-rate]', '0.5');
    await page.fill('[data-value-per-conversion]', '10');
    await page.locator('[data-calculate]').click();
    const status = await page.locator('[data-line-status]').textContent();
  ...
  test('pushes calculator_result event to dataLayer', async ({ page }) => {
    ...
    expect(event.calculator_name).toBe('SEO ROI Calculator');
    expect(typeof event.result_value).toBe('number');
    expect(typeof event.monthly_cost).toBe('number');
...
test.describe('Finance hub lists the SEO ROI Calculator', () => {
  test('SEO ROI Calculator shows on the Finance hub', async ({ page }) => {
