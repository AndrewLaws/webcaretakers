...
- Set up weekly export of Performance data (queries, pages, CTR, position) so we have a local record beyond the 16-month GSC window. Use a Search Console API service account with **read-only** scope (`https://www.googleapis.com/auth/webmasters.readonly`); do not grant write or Indexing API access
...
**Status:** [~] In progress. 2026-04-30: in-link audit clean — zero orphans across 110 calculators, lowest in-link count is 1, top pages get 12–15 (word-count, unit-converter, read-time-calculator, photo-resizer, compound-interest). Found and fixed a duplication bug: `inject-seo-schema.js` auto-generated a `class="related-calcs"` block on every page but its dedup check missed pages with hand-picked `<!-- related-calculators-block -->` blocks, so 53 pages shipped with two "Related calculators" sections. Widened the dedup check (also looks for the hand-picked marker) and ran a one-off cleanup script (`scripts/strip-duplicate-related-calcs.js`) to strip the duplicate auto blocks. 53 pages cleaned, 0 skipped, all tests pass. Hand-picked block rollout in progress: writing (2/2 done) and business (2/2 done) converted with intent-driven anchor text and a one-line frame. Remaining 53 by category: cybersecurity 8, math 6, broadband 6, ai 6, seo 4, property 4, productivity 4, images 4, fun 4, conversions 4, health 3. 2026-05-01: also trimmed the homepage Featured Calculators section from 19 cards (heavily skewed: 7 finance, 7 wedding) down to 15 (one per live category) per the "homepage links to the top calculator in each live category, not all of them" rule. New picks: ai/LLM Token Usage, broadband/Bandwidth, business/Domain Renewal, conversions/Cooking Measurements, cybersecurity/Password Strength, finance/US Mortgage, fun/Random Name Picker, health/BMI, images/Photo Resizer, math/Percentage, productivity/iCal Event Generator, property/UK Rent vs Buy, seo/URL Slug Generator, wedding/Wedding Budget, writing/Word Count. Pattern documented: hand-picked blocks reuse the `related-calcs` styling hooks for visual consistency but carry the `<!-- related-calculators-block -->` marker comment immediately above the section so the inject-seo-schema dedup check and the strip script's negative-lookbehind regex both treat them correctly. 2026-05-02: confirmed cross-link UK/US sibling pairs are already in place (the only live UK↔US pair is finance mortgage; both sides carry the `country-switch` callout). Hand-picked rollout finished across all 14 categories on the same day: health 3/3, conversions 4/4, productivity 4/4, seo 4/4, property 4/4, images 4/4, fun 4/4, math 6/6, ai 6/6, broadband 6/6, cybersecurity 8/8 (53 pages converted in this push, on top of the 12 already done in writing, business and elsewhere). Auto-block duplicates stripped on every page. JSON-LD validator clean across all 132 pages after each batch. **Status: complete.** All 110 calculators carry intent-driven Related Calculators blocks with marker comments, four-sibling lists chosen for cross-coverage within (and where useful, across) categories, and a one-line frame paragraph framing the picks. Future drift will be caught by the monitoring cadence in #11.
...
**Status:** [~] In progress. 2026-04-30: audit revealed the long-form work was already done — every one of the 15 hubs has 593 to 982 words of long-form content, well inside the 600 to 1,200 target. The real gap is hub-level FAQ blocks: zero of 15 hubs had one. Built the pattern on the health hub: five `<details>` questions covering the top first-principles queries (medical-advice disclaimer, why-different-answers, which-to-start-with, units, data-privacy) plus matching FAQPage JSON-LD with five Question/Answer pairs. Placed between the calculator grid and the closing caveats long-form section so the page reads intro → long-form → grid → FAQ → caveat-closer. Validates clean against the schema linter. Note: questions are first-principles guesses, not GSC-derived; once GSC data flows (task #1) we will refine each hub's FAQ to match real queries. FAQ rollout complete: 15 of 15 hubs now carry a hand-written hub-level FAQ block plus matching FAQPage JSON-LD. Each hub got five questions tuned to that category's first-principles visitor intent, with internal links into the relevant calculators where natural for extra in-link flow. Categories shipped 2026-05-01: ai (tokens, API costs, fine-tuning, RAG, AI feature budgeting); business (domain renewal hike, in-house vs MSP break-even, freelance day rate, project break-even, SEO/email ROI); conversions (US vs metric, fluid vs weight oz, density caveats, official factors, privacy); cybersecurity (password strength, passphrases, breach crack times, TOTP window, privacy); fun (randomness fairness, Decision Wheel, raffle suitability, Magic 8-Ball weighting, age in months); images (file format choice, web-shrinking, privacy, social media dimensions, WebP vs AVIF); math (percentages vs percentage points, mental discount maths, BIDMAS, fractions, privacy); productivity (cross-app calendar events, Pomodoro length, deadline slips, iCal vs ICS, sharing without accounts); property (rent vs buy, stamp duty bands, rental yield, mortgage overpayment, Section 24); seo (meta length, indexed-but-not-ranking, cannibalisation, slug rules, snippet preview accuracy); wedding (UK budget, save-the-date timing, budget split, drinks per guest, supplier booking lead time); writing (word count targets, Flesch ease, character limits, privacy, headline scoring). All 132 pages validate clean. Refinement to actual GSC top queries per hub URL is task #1 follow-up. Remaining on this section: sub-category groupings inside large hubs, refine FAQ content once GSC data flows.
...
**Status:** [~] In progress. 2026-05-01: built `scripts/measure-cwv.js` (with 10 unit tests) that spins up a local static server, drives Playwright Chromium against a 10-page sample (homepage, all-calc index, two hubs, six calculators of varied complexity), captures FCP/LCP/CLS/transfer-size/request-count, aggregates per-template medians and max, and writes `cwv-baseline.json`. Exposed via `npm run measure:cwv` and `npm run check:cwv`. Per-template budgets live at `scripts/cwv-budgets.json` and are checked against the worst page in each template (max, not median). Baseline numbers (lab, localhost, so no network factored): LCP 76 to 176ms across all pages (Google Good threshold is 2500ms — site is 14 to 30x under it), CLS 0 on every page except `/calculators/writing/word-count/` at 0.038 (still well under the 0.1 Good threshold), transfer 860 to 918 KB per page (consistent across templates, mostly the GTM script and shared CSS), 7 to 9 requests per page. All four templates pass the initial budgets with deliberate headroom so the budget catches genuine regressions rather than firing on noise. Baseline JSON is gitignored as a regenerated artefact; commit specific snapshots manually with `git add -f cwv-baseline.json` if you want to record a moment in time. 2026-05-01 follow-up: word-count CLS investigated and root-caused. The 0.038 was not specific to word-count; it was caused by `main.js` dynamically prepending the `.site-search-bar` (~50px tall) before the site header after first paint, shifting all subsequent layout. Lab measurement caught it on word-count by timing alone; every page actually had the shift. Fix: built `scripts/inject-search-bar.js` that pre-renders the search-bar markup at build time on every page. The JS injection in main.js already guards with `if (document.querySelector('[data-site-search]')) return;`, so once the markup is in static HTML the JS becomes a no-op. Ran across 132 pages. Re-measured: CLS now 0 across all sampled templates. Still to do: wire `npm run check:cwv` into a pre-deploy step, re-baseline once GSC field data flows.
...
- `scripts/post-deploy-index-ping.js` diffs `sitemap.xml`, POSTs new URLs to IndexNow for Bing/Yandex/Seznam, logs every submission. Google is *not* in scope: see the corrected stance below.

**Status:** [x] Done for IndexNow; Google indexing is intentionally manual. 2026-04-30: built `scripts/post-deploy-index-ping.js` with 12 unit tests covering sitemap parsing, URL diff, payload shape and chunking. Walks `site/sitemap.xml` (treated as a sitemapindex pointing at the per-category urlsets), collects all canonical URLs, diffs against `.indexnow-snapshot.json`, and POSTs added URLs (chunked at the 10k IndexNow limit) to `https://api.indexnow.org/IndexNow`. Every run appends to `.indexnow-log.jsonl`. First run writes the baseline snapshot without pinging unless `--initial-publish` is set, so we cannot accidentally spam the entire catalogue. `--dry-run` flag for safe inspection. Exposed via `npm run index:ping` and `npm run index:ping:dry`. 2026-05-02: provisioned `INDEXNOW_KEY` in `.env`, committed the ownership verification file at `site/<key>.txt` so it deploys with the next push. After the deploy, run `npm run index:ping` once to baseline the snapshot. **Google indexing was reviewed against ToS on 2026-05-02 and the original plan was corrected:** the Google Indexing API is officially restricted to JobPosting and BroadcastEvent schemas, calculator pages do not qualify, and submitting them anyway is a policy violation that risks the GSC property. The submitToGsc stub has been removed from the script and the header comments now state Indexing API is permanently out of scope. GSC sitemap submission is a one-off in the UI (no value in automating). For specific pages that need faster Google crawl, use GSC URL Inspection → Request Indexing (capped ~10/day, fully sanctioned). The Google service account, when provisioned, will be Search Console **read-only** for reporting/audit, not indexing.
